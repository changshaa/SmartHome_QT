/****************************************************************************
** Meta object code from reading C++ file 'device_curtain.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.3.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../myServer/device/device_curtain.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'device_curtain.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.3.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_DeviceCurtain_t {
    QByteArrayData data[12];
    char stringdata[170];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_DeviceCurtain_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_DeviceCurtain_t qt_meta_stringdata_DeviceCurtain = {
    {
QT_MOC_LITERAL(0, 0, 13),
QT_MOC_LITERAL(1, 14, 12),
QT_MOC_LITERAL(2, 27, 0),
QT_MOC_LITERAL(3, 28, 6),
QT_MOC_LITERAL(4, 35, 8),
QT_MOC_LITERAL(5, 44, 19),
QT_MOC_LITERAL(6, 64, 20),
QT_MOC_LITERAL(7, 85, 17),
QT_MOC_LITERAL(8, 103, 18),
QT_MOC_LITERAL(9, 122, 16),
QT_MOC_LITERAL(10, 139, 15),
QT_MOC_LITERAL(11, 155, 14)
    },
    "DeviceCurtain\0sig_sendData\0\0sockID\0"
    "sendData\0on_btn_down_pressed\0"
    "on_btn_down_released\0on_btn_up_pressed\0"
    "on_btn_up_released\0slot_dataChanged\0"
    "slot_slider_OFF\0slot_slider_ON"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_DeviceCurtain[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    2,   54,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    0,   59,    2, 0x08 /* Private */,
       6,    0,   60,    2, 0x08 /* Private */,
       7,    0,   61,    2, 0x08 /* Private */,
       8,    0,   62,    2, 0x08 /* Private */,
       9,    0,   63,    2, 0x08 /* Private */,
      10,    0,   64,    2, 0x08 /* Private */,
      11,    0,   65,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int, QMetaType::QString,    3,    4,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void DeviceCurtain::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        DeviceCurtain *_t = static_cast<DeviceCurtain *>(_o);
        switch (_id) {
        case 0: _t->sig_sendData((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2]))); break;
        case 1: _t->on_btn_down_pressed(); break;
        case 2: _t->on_btn_down_released(); break;
        case 3: _t->on_btn_up_pressed(); break;
        case 4: _t->on_btn_up_released(); break;
        case 5: _t->slot_dataChanged(); break;
        case 6: _t->slot_slider_OFF(); break;
        case 7: _t->slot_slider_ON(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (DeviceCurtain::*_t)(int , QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&DeviceCurtain::sig_sendData)) {
                *result = 0;
            }
        }
    }
}

const QMetaObject DeviceCurtain::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_DeviceCurtain.data,
      qt_meta_data_DeviceCurtain,  qt_static_metacall, 0, 0}
};


const QMetaObject *DeviceCurtain::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *DeviceCurtain::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_DeviceCurtain.stringdata))
        return static_cast<void*>(const_cast< DeviceCurtain*>(this));
    return QWidget::qt_metacast(_clname);
}

int DeviceCurtain::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void DeviceCurtain::sig_sendData(int _t1, QString _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
